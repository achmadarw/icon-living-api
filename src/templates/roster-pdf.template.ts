...
// @ts-nocheck
//
// Port APA ADANYA dari `tia_security_backend/src/templates/roster-pdf.template.js`.
// Sengaja tidak dirapikan agar hasil PDF identik dengan TIA.
// Perbedaan: sumber logo dan tanda tangan diambil dari parameter `data`.
import fs from 'fs';
...
function generateRosterPDFHTML(data) {
    ...
    const logoPath = data.logoDir || path.join(process.cwd(), 'assets', 'logos');
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="header-text">
                <h3>${data.headerLine1 || 'KABUPATEN BOGOR'}</h3>
                <h3>${data.headerLine2 || 'KECAMATAN CIBINONG KELURAHAN KARADENAN'}</h3>
                <h3>${data.headerLine3 || 'PAGUYUBAN THE ICON ACROPOLIS RT010/RW018'}</h3>
                <h2>${month}</h2>
            </div>
            ${
                satpamLogo
        ...
        <div class="content-wrapper">
            ...
            <div class="signature-right">
                <div class="coordinator-section">
                    <div class="coordinator-title">${data.signerNames || 'DARWAN / AGUS DARMAWAN'}<br/>${data.signerTitles || 'KETUA PAGUYUBAN / KOORDINATOR KEAMANAN'}</div>
                </div>
            </div>
        </div>
        </div>
